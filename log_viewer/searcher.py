...
def search_lines(lines: List, keyword: str, context: int = 0) -> Tuple[List[Match], Dict[str, Any]]:
    """Return matches and a summary dict.

    Matches are returned with context_before and context_after lists of lines (tuples of line_no, text).
    """
    matches: List[Match] = []
    total = len(lines)
    earliest = None
    ...
    for i, line in enumerate(lines):
        if keyword in line.text:
            ...
            # create a highlighted version for display, keep original in raw_text
